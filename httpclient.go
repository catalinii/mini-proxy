...
func NewHTTPConnectDialer(proxyUrlStr string) (*HTTPConnectDialer, error) {
	...
	if proxyUrl.User != nil {
		if proxyUrl.User.Username() != "" {
			...
			client.DefaultHeader.Set("Authorization", "Basic "+
...
func (c *HTTPConnectDialer) Dial(network, address string) (net.Conn, error) {
	cc, _, err := c.DialContext(context.Background(), network, address)
	return cc, err
}
...
func (c *HTTPConnectDialer) DialContext(ctx context.Context, network, address string) (net.Conn, http.Header, error) {
	req := (&http.Request{
		Method: "CONNECT",
	...
	connectHttp2 := func(rawConn net.Conn, h2clientConn *http2.ClientConn) (net.Conn, http.Header, error) {
		req.Proto = "HTTP/2.0"
		req.ProtoMajor = 2
		req.ProtoMinor = 0
		pr, pw := io.Pipe()
		...
		//		log.Printf("Connected using HTTP2 to %v", address)
		return NewHttp2Conn(rawConn, pw, resp.Body), resp.Header, nil
	}

	connectHttp1 := func(rawConn net.Conn) (net.Conn, http.Header, error) {
		req.Proto = "HTTP/1.1"
		req.ProtoMajor = 1
		req.ProtoMinor = 1
		...
		//		log.Printf("Connected using HTTP1 to %v", address)

		return rawConn, resp.Header, nil
	}
	...
	if c.EnableH2ConnReuse {
		...
		if c.cachedH2ClientConn != nil && c.cachedH2RawConn != nil {
			if c.cachedH2ClientConn.CanTakeNewRequest() {
				...
				c.cacheH2Mu.Unlock()
				unlocked = true
				proxyConn, header, err := connectHttp2(rc, cc)
				if err == nil {
					return proxyConn, header, err
				}
	...
	default:
		return nil, nil, errors.New("scheme " + c.ProxyUrl.Scheme + " is not supported")
	}
	...
	case "h2":
		...
		proxyConn, header, err := connectHttp2(rawConn, h2clientConn)
		if err != nil {
			rawConn.Close()
			return nil, nil, err
		}
	...
	default:
		rawConn.Close()
		return nil, nil, errors.New("negotiated unsupported application layer protocol: " +
			negotiatedProtocol)
	}
}
